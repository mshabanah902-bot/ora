...
export default function CartDrawer({ open, items, onClose, onChange, onClear }: { open: boolean; items: CartItem[]; onClose: () => void; onChange: (lineId: string, delta: number) => void; onClear: () => void }) {
  ...
  const confirmOrder = async () => {
    ...
    const order = { customer, region, items: items.map(({ id, name, price, quantity, selectedColor, selectedSize }) => ({ id, name, color: selectedColor, size: selectedSize, price, quantity })), total };
    try { await saveOrder(order); } catch { return; }
    const text = `طلب جديد من ORA%0Aالاسم: ${customer.name}%0Aالهاتف: ${customer.phone}%0Aالموقع: ${customer.address}%0Aالمنطقة: ${region}%0A${items.map((item) => `${item.name} (${item.selectedColor} - ${item.selectedSize}) × ${item.quantity} = ₪${item.price * item.quantity}`).join('%0A')}%0Aالتوصيل: ₪${delivery[region]}%0Aالإجمالي: ₪${total}`;
  ...
  return <AnimatePresence>{open && <><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="fixed inset-0 bg-black/40 z-[60]" /><motion.aside initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 26 }} dir="rtl" className="fixed top-0 right-0 h-full w-full max-w-md bg-[#faf8f5] z-[61] shadow-2xl p-5 sm:p-7 overflow-y-auto">
    ...
    {!items.length ? <p className="text-center py-16 text-charcoal-500">{language === 'he' ? 'הסל ריק כרגע' : 'السلة فارغة حاليًا'}</p> : <><div className="space-y-4">{items.map((item) => <div key={item.lineId} className="flex gap-3 items-center bg-white rounded-2xl p-3 shadow-sm"><img src={item.image} className="w-16 h-20 object-cover rounded-xl" /><div className="flex-1"><b>{item.name}</b><p className="text-xs text-charcoal-500">{item.selectedColor} · ₪{item.price}</p><div className="flex items-center gap-2 mt-2"><button onClick={() => onChange(item.lineId, -1)} className="p-1 rounded-full bg-ora-100"><Minus size={14} /></button><span>{item.quantity}</span><button onClick={() => onChange(item.lineId, 1)} className="p-1 rounded-full bg-ora-100"><Plus size={14} /></button></div></div><button onClick={() => onChange(item.lineId, -item.quantity)} className="text-red-400"><Trash2 size={17} /></button></div>)}</div>
