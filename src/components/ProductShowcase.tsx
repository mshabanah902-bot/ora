  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="group">
      ...
      <div className="px-1">
        ...
        <div className="mt-3"><span className="text-xs text-charcoal-500">{language === 'he' ? t('availableSizes') : 'النمر المتوفرة:'}</span><div className="flex flex-wrap gap-1.5 mt-1">{colorSizes.map((item) => <button key={item.name} disabled={!item.available} onClick={() => setSize(item.name)} className={`min-w-8 px-2 py-1 rounded-lg border text-xs font-semibold transition-all ${item.available ? 'availability-available' : 'availability-unavailable'} ${size === item.name ? 'ring-2 ring-ora-300' : ''} disabled:opacity-100 disabled:line-through`} aria-label={`${language === 'he' ? t('size') : 'نمرة'} ${item.name}`}>{item.name}</button>)}</div></div>
